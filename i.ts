...
let prevFrameTimestamp: DOMHighResTimeStamp;
let position: number = 10;
let shipX: number = 70;
let shipY: number = 70;
let shipAngle: number = 0;
let shipSpeed: number = 1; // pixels per second
let downLeft: boolean = false;
let downRight: boolean = false;
let downForward: boolean = false;

const drawSpaceShip = (x: number, y: number, angle: number) => {
  const size = 20;
...
const updateSpaceship = (delta: number) => {
  ...
  if (downLeft) shipAngle -= delta * 0.003;
  if (downRight) shipAngle += delta * 0.003;
  if (downForward) shipSpeed += delta * 0.002;
};

const handleEvent = ({ kind, key }: { kind: string; key: number }) => {
  if (kind === "keydown" || kind === "keyup") {
    const isDown = kind === "keydown";
    if (key === 37) downLeft = isDown;
    if (key === 39) downRight = isDown;
    if (key === 38) downForward = isDown;
  }
};

["keydown", "keyup"].map((evName) =>
  document.body.addEventListener(evName, (ev) =>
    handleEvent({ kind: evName, key: (ev as any).keyCode })
  )
);
...
const nextFrame = (now: DOMHighResTimeStamp) => {
  ...
  const delta: number = now - (prevFrameTimestamp || now);
